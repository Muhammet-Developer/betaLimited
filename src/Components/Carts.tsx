...
const Carts = ({ props }: IProps) => {
  ...
  const count = async () => {
    ...
    const newData = data === 'Cart is empty.' ? 0 : data.length;
    dispatch(SET_BASKET_LENGTH(newData));
  };
  ...
  return (
    <Card sx={{ maxWidth: 400, position: 'relative',borderRadius:3 }} style={{ marginLeft: 'auto', marginRight: 'auto' }}>
      <CardMedia
        component='img'
        alt={props?.name}
        height='auto'
